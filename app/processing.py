...
def edit_page_json(config_name, questionnaire, id,index,title):
    try:
...
def edit_question_json(config_name, questionnaire, page_identifier, question_identifier, question_index, question, answer_type):
    try:
        config_data = json.load(open(config_name,'r', encoding='utf-8'))
...
def add_answer_to_json(config_name, questionnaire, page_identifier,question_identifier,answer_index,text,weight):
    try:
        config_data = json.load(open(config_name,'r'), encoding='utf-8')

        #print(config_data, flush=True)

        found_id, page_id = get_id_json(config_data['questionnaire'][questionnaire]['pages'], page_identifier)
